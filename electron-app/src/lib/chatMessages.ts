...
export const TOOLS_RUNWARE_IMAGE_HINT = `You have three image tools: generate_image, edit_image_runware, and image_recall. Use generate_image only for fresh text-to-image creation. Use edit_image_runware when the user asks to modify existing images; pass prompt plus either reference_image_indexes (1-based internal catalog indexes, index 1 is most recent) and/or reference_image_paths (absolute paths from chat history). Use image_recall to retrieve historical image bytes from the internal catalog when you need vision-style analysis on older images in the current turn (it also accepts indexes and/or paths). Always use the selected model profile resolution from Options (do not set width/height in tool args). Keep steps/cfg at the selected model profile defaults unless the user explicitly requests changing them. Never print raw base64 in chat replies. After image-generation/edit output, provide a concise result and include the generated image URL.`
