...
export function FilePreview({ filePath, content }: Props) {
  return (
    <div className="flex min-h-0 flex-1 flex-col rounded border border-void-muted/30 bg-void-black/30 p-2">
      <div className="mb-2 shrink-0 text-xs font-mono text-neon-green">
        PREVIEW {filePath ? `- ${filePath}` : ''}
      </div>
      <pre className="min-h-0 flex-1 overflow-auto whitespace-pre-wrap break-words text-xs text-void-light">
