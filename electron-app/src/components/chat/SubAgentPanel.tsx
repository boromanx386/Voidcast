...
export type SubAgentActivityCardProps = {
  ...
  onDismiss: () => void
  /**
   * Nested inside an assistant message bubble (no outer message-container).
   * Default standalone bubble layout for legacy uses.
   */
  embedded?: boolean
}

/** Collapsible vision / explore / workers activity card. */
export function SubAgentActivityCard({
  ...
  const body = (
    <>
      <div className="flex items-center justify-between gap-2 border-b border-void-muted/25 px-1 pb-2">
        <button
          type="button"
          onClick={() => onCollapse(!panel.collapsed)}
  ...
  if (embedded) {
    return (
      <div
        className="rounded border border-neon-cyan/30 bg-void-black/40 px-2 py-2"
        role="region"
